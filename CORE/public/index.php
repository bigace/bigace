...
if($bg_uo->isValidUser())
{
	if($bg_uo->isActive())
	{
	    ...
	    if($GLOBALS['_BIGACE']['PARSER']->isSanitized() && $GLOBALS['_BIGACE']['PARSER']->getCommand() != null && strlen(trim($GLOBALS['_BIGACE']['PARSER']->getCommand())) > 0) {
	    	import('classes.exception.ExceptionHandler');
		    import('classes.exception.NotFoundException');

	        ExceptionHandler::processCoreException( new NotFoundException(404, 'Invalid request detected: ' . $_SERVER['REQUEST_URI']) );
        }
        else {
            // PROCESSING COMMAND INTERPRETER
            $cmd = $GLOBALS['_BIGACE']['PARSER']->getCommand();
            $COMMAND_FILE = _BIGACE_DIR_ROOT . '/system/command/' . $cmd . '.cmd.php';
            ...
            else /* Command could not be found */
        	{
			    import('classes.exception.ExceptionHandler');
			    import('classes.exception.NotFoundException');
		        ExceptionHandler::processCoreException( new NotFoundException(404, 'CMD not found ['. $cmd .'] using ['.$_SERVER['REQUEST_URI'].']') );
        	}
            unset ($COMMAND_FILE);
        }
	}
	...
	{
		// User is DEACTIVATED, but allow login action to change to another user
		$cmd = $GLOBALS['_BIGACE']['PARSER']->getCommand();
		$action = $GLOBALS['_BIGACE']['PARSER']->getAction();
		if ($cmd === 'application' && $action === 'auth') {
			include_once (_BIGACE_DIR_ROOT . '/system/command/application.cmd.php');
		}

...
{$contents}
{if $nav}
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tutorialNav">
<tr>
<td width="33%" align="left" valign="top">{if $prev}<a href="{$prev}" class="tutPrev">{/if}Prev{if $prev}</a>{/if}</td>
<td width="34%" align="center" valign="top">{if $up}<a href="{$up}" class="tutUp">Up</a>{else}&nbsp;{/if}</td>
<td width="33%" align="right" valign="top">{if $next}<a href="{$next}" class="tutNext">{/if}Next{if $next}</a>{/if}</td>
</tr>
<tr>
